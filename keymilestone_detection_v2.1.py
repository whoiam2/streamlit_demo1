...
topics1 = ['All','Environmental Exploitation', 'Economic Theories', 'Environmental Degradation','History','Recycle']
topics2 = ['All','Sustainability', 'Sustainable Living', 'Government Policies', 'Regulations, and Sustainability','Reduce']
topics3 = ['All','Climate Changes', 'Renewable Energy', 'Fossil Fuels','Reuse','Electrical Engineering']
topics4 = ['All','Water Scarcity, Privatization', 'Deforestation', 'Industrial Farming','Data Science']
topics5 = ['All','Sustainable Agricultures', 'Fishing and Aquaculture', 'Waste Management','Coding','Policy Law']
...
topic_text = 'Topics covered by the relevant materials provided'
...
images = []
for file in [
        "Assets/Image_select/1assignment1.png",
        "Assets/Image_select/2assignment2.png",
        "Assets/Image_select/3essay1.png",
        "Assets/Image_select/4midterm1.png",
        "Assets/Image_select/5final1.png",
    ]:
...
clicked = clickable_images(
    ...
    div_style={"display": "block", "justify-content": "center", "flex-wrap": "wrap", "color" :'blue'},
    img_style={"margin": "5px", "height": "60px"},
)
completed_images = []
for file in [
        "Assets/Image_select/1assignment1_c.png",
        "Assets/Image_select/2assignment2_c.png",
        "Assets/Image_select/3essay1_c.png",
        "Assets/Image_select/4midterm1_c.png",
        "Assets/Image_select/5final1_c.png",
    ]:
    with open(file, "rb") as image:
        encoded = base64.b64encode(image.read()).decode()
        completed_images.append(f"data:image/jpeg;base64,{encoded}")
if clicked>=0:
    st.sidebar.image(completed_images[int(clicked)],caption='Study schedule selected')
    doc_vol_cnt = 10+(clicked if clicked // 2 ==0 else -clicked)
    uq_vol_cnt = 7+(clicked if clicked // 2 ==0 else -clicked)
    st.sidebar.write(f'Relevant document: {doc_vol_cnt}')
    st.sidebar.write(f'Relevant user questions: {uq_vol_cnt}')
    nav = ['Assignment - week of 2024-06-03', 'Assignment - week of 2024-06-10','Essay - week of 2024-06-17','Midterm - week of 2024-07-08','Final - week of 2024-08-19'][clicked]
else:
    st.write('Please select a milestone')
    nav = 'None'
...
if nav.split(' ')[0] == 'Assignment':
    st.markdown(f'''<p style='color: black; font-size:20px;' > Your {nav.split(' ')[0]} is coming up soon, start preparing for it now </p>''', unsafe_allow_html=True)
    ...
    with tab5:
        @st.experimental_dialog("Create a custom flashcards")
        def generate_fc():
            with st.container():
    ...
    with tab4:
        ...
        def generate_pt():
            with st.container():
                st.write("Create a custom practice test from our library or highly relevant documents or upload your own document!")
                generate_type = st.radio('Choose between our library or upload', ['Course Hero Library','Upload your own document'],horizontal=True,index=None)
                if generate_type =='Course Hero Library':
                    ...
                    if selected_method == 'By Topics':
                        ...
                        if st.button('Generate'):
                                st.write("Practice test generated successfully")
                                open_page(ai_test_prep_url)
                if generate_type == 'Upload your own document':
                    uploaded_file = st.file_uploader("Choose a file")
                    if uploaded_file is not None:
                        file_uploaded = True
                    file_uploaded = st.checkbox("By pass upload step")
                    if file_uploaded:
                        if st.button('Generate practice test with my own document'):
                            st.write("Practice test generated successfully")
                            open_page(ai_test_prep_url)
...
if nav.split(' ')[0] == 'Midterm' or nav.split(' ')[0] == 'Final':
    st.markdown(f'''<p style='color: black; font-size:20px;' > Your {nav.split(' ')[0]} is coming up soon! Practice with our practice test prep tool! </p>''', unsafe_allow_html=True)
    # st.write(f'Prepare this week for your {nav.split(' ')[0]} coming up in the follow weeks!')
    # st.markdown(f'''<a style='text-align: center; color: black; font-size:28px;' href='{ai_essay_url}' > AI essay helper </a>''', unsafe_allow_html=True)
